...
class PackageExpr {
public:
  PackageExpr(const PackageExprType &type = PackageExprType::None)
      : expr_type_(type) {}
...
public:
  ...
  IfElsePackage(PropExprPtr &cond, PackageExprPtr &body,
                PackageExprPtr &else_body)
      : PackageExpr(PackageExprType::If), cond_(cond), body_(body),
...
public:
  ...
  PackageExprUnary(UnaryType type, PackageExprPtr &expr)
      : PackageExpr(PackageExprType::Unary), type_(type), expr_(expr) {}
...
public:
  ...
  PackageExprBinary(BinaryType type, PackageExprPtr &lhs, PackageExprPtr &rhs)
      : PackageExpr(PackageExprType::Binary), type_(type), lhs_(lhs),
        rhs_(rhs) {}
...
public:
  PackageExprAtomWithProp(AtomType type, const std::string &pack_name,
                          const PropName &prop_name, const std::string &string)
      : PackageExpr(PackageExprType::AtomWithProp), type_(type),
        pack_name_(pack_name), prop_name_(prop_name), string_(string) {}

  PackageExprAtomWithProp(const std::string &pack_name,
                          const PropName &prop_name, const std::string &string)
