...
namespace depkit
{
	...
	void CodeGenZ3::codegen(TermsPtr& terms)
	{
	...
	void CodeGenZ3::buildSolver()
	{
		solver_ = std::make_shared<z3::solver>(ctx_);
	}
	...
	void CodeGenZ3::addVersionConstraint()
	{
		...
		for (auto& packpair: packs_)
		{
			...
			for (unsigned i = 1; i < versions.size(); ++i)
				constraint = constraint || (version == ctx_.real_val(versions[i].c_str()));
			solver_->add(z3::implies(package, constraint));
		} 
	}

	void CodeGenZ3::VisitDefinitions(TermsPtr& terms)
	{
		current_state_ = CurrState::Definition;
		for (auto& def : terms->definitions_) Visit(def);
	}

	void CodeGenZ3::VisitDependencies(TermsPtr& terms)
	{
		current_state_ = CurrState::Dependency;
		for (auto& dep : terms->dependencies_) Visit(dep);
	}

	void CodeGenZ3::VisitRequirements(TermsPtr& terms)
	{
		current_state_ = CurrState::Requirement;
		for (auto& req : terms->requirements_) Visit(req);
	}
	
	void CodeGenZ3::Visit(DefinitionPtr& def)
	{
		Visit(def->identification_);
	}

	void CodeGenZ3::Visit(DependencyPtr& dep)
	{
	...
	void CodeGenZ3::Visit(RequirementPtr& req)
	{
		Visit(req->requirement_);
	}

	void CodeGenZ3::Visit(IdentificationPtr& id)
	{
		curr_pack_name_ = id->name_;
		if (current_state_ == CurrState::Definition) {
			const z3::expr name = 
				ctx_.bool_const(curr_pack_name_.c_str());					// A as bool
			const z3::expr version = 
				ctx_.real_const((curr_pack_name_ + ".version").c_str());	// A.version as float
	...
	void CodeGenZ3::Visit(PropertyListPtr& plist)
	{
		...
		if (current_state_ == CurrState::Definition)
		{
			...
			for (auto& f : plist->features_) {
				const z3::expr feat = ctx_.bool_const((curr_pack_name_ + ".feat." + f).c_str());
				p.features_.emplace(f, feat);		// A.feat.xxx as bool
			}
			for (auto& b : plist->backends_) {
				const z3::expr backend = ctx_.bool_const((curr_pack_name_ + ".backend." + b).c_str());
				p.backends_.emplace(b, backend);	// A.backend.xxx as bool
			}
		}
		else if (current_state_ == CurrState::Requirement)
		{
			ASSERT(plist->versions_.size() <= 1, "Cannot specify more than one version in requirement.");
			if (plist->versions_.size() == 1)
			{
				const char* version_str = plist->versions_.begin()->c_str();
				solver_->add(p.version_ == ctx_.real_val(version_str));	// (assert (== A.version mm.nn))
			}
			for (auto& f : plist->features_)
			{
	...
	z3::expr CodeGenZ3::Visit(PropExprPtr& expr)
	{
	...
	z3::expr CodeGenZ3::Visit(PackageExprPtr& expr)
	{
		switch (expr->expr_type_)
		{
